...
std::optional<std::string> FunctionPointerObjective::validate() const {
    ...
    if(this->numParameters() > 0 && 
        ...
        return "Number of parameters does not match the parameters vector size, or the vector has not been set";
    }
...
FunctionPointerObjective::FunctionPointerObjective(
        int numVariables,
        int numInequalityConstraints,
        int numEqualityConstraints,
        int numParameters): _numVariables(numVariables),
...
const Eigen::VectorXd FunctionPointerObjective::gradient(const Eigen::VectorXd& state) const {
    Eigen::VectorXd vec(this->numVariables());
    (*this->gradientFunction)(state.data(), this->_parameters->data(), vec.data());
    return vec;
}

void FunctionPointerObjective::setGradientFunction(GradientFunction functionPtr) {
    this->gradientFunction = functionPtr;
}

const Eigen::MatrixXd FunctionPointerObjective::hessian(const Eigen::VectorXd& state) const {
    Eigen::MatrixXd mat(this->numVariables(), this->numVariables());
    (*this->hessianFunction)(state.data(), this->_parameters->data(), mat.data());
    return mat;
}
...
std::optional<const Eigen::MatrixXd> FunctionPointerObjective::equalityConstraintMatrix() const {
    if(this->numEqualityConstraints() > 0) {
        Eigen::MatrixXd mat(this->numEqualityConstraints(), this->numVariables());
...
void FunctionPointerObjective::setEqualityMatrixFunction(EqualityMatrixFunction functionPtr) {
    this->equalityMatrixFunction = functionPtr;
}

std::optional<const Eigen::VectorXd> FunctionPointerObjective::equalityConstraintVector() const {
    if(this->numEqualityConstraints() > 0) {
...
void FunctionPointerObjective::setEqualityVectorFunction(EqualityVectorFunction functionPtr) {
    this->equalityVectorFunction = functionPtr;
}

double FunctionPointerObjective::inequalityConstraintsValue(const Eigen::VectorXd& state) const {
    if(this->numInequalityConstraints() > 0) {
        double value;
        (*this->inequalityValueFunction)(state.data(), this->_parameters->data(), &value);
        return value;
    } else {
...
void FunctionPointerObjective::setInequalityValueFunction(InequalityValueFunction functionPtr) {
    this->inequalityValueFunction = functionPtr;
}

const Eigen::VectorXd FunctionPointerObjective::inequalityConstraintsGradient(const Eigen::VectorXd& state) const {
    if(this->numInequalityConstraints() > 0) {
        Eigen::VectorXd vec(this->numVariables());
        (*this->inequalityGradientFunction)(state.data(), this->_parameters->data(), vec.data());
        return vec;
    } else {
...
void FunctionPointerObjective::setInequalityGradientFunction(InequalityGradientFunction functionPtr) {
    this->inequalityGradientFunction = functionPtr;
}

const Eigen::MatrixXd FunctionPointerObjective::inequalityConstraintsHessian(const Eigen::VectorXd& state) const {
    if(this->numInequalityConstraints() > 0) {
        Eigen::MatrixXd mat(this->numVariables(), this->numVariables());
        (*this->inequalityHessianFunction)(state.data(), this->_parameters->data(), mat.data());
        return mat;
    } else {
...
void FunctionPointerObjective::setInequalityHessianFunction(InequalityHessianFunction functionPtr) {
