...
TEST(SymbolicEqualityTests, BasicLinearization) {
    ...
    // What  we expect
    DenseMatrix expectedMat =
            DenseMatrix(2, 3,
                        {integer(0), integer(-3), integer(1), rational(1, 2),
                         integer(0), integer(0)});

    DenseMatrix expectedVector = DenseMatrix(
            2, 1,
            {integer(4), sub(integer(7), div(a.get_basic(), integer(2)))});
